...
# and repo root for Recommendation agents (not yet in worker_agents).
# Paths are added per-route in each route_* function so imports resolve correctly.
_base_file = os.path.abspath(__file__)
_base_dir = os.path.dirname(os.path.dirname(_base_file))  # agentic-core
_repo_root = os.path.dirname(os.path.dirname(_base_dir))  # repo root
_workers = os.path.join(_base_dir, 'worker_agents')
base_dir = _base_dir  # for any code that still uses base_dir

...
from whatsapp.models import WhatsAppMessage, WhatsAppResponse, AuthState
from whatsapp.auth_handler import auth_handler
...
class MessageHandler:
    ...
    def _init_orchestrator(self):
        ...
        try:
            # Try to import orchestrator client (new structure: backend/agentic-core/master_agent, else repo Orchestrator)
            _base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # backend/integrations
            _backend = os.path.dirname(_base)
            _repo = os.path.dirname(_backend)
            for orchestrator_path in [
                os.path.join(_backend, "agentic-core", "master_agent"),
                os.path.join(_repo, "Orchestrator"),
            ]:
                if os.path.isdir(orchestrator_path) and orchestrator_path not in sys.path:

...
async def generate_qr_session():
    """
    Generate a new QR code session for kiosk
    User scans this QR code with their WhatsApp/device to link their account
    """
    cleanup_expired_sessions()
    
    # Generate unique session ID
    session_id = f"KIOSK-{uuid.uuid4().hex[:12].upper()}"
    ...
    # Generate session URL (what user will scan)
    # In production, this would be a deep link or WhatsApp link
    session_url = f"{FRONTEND_URL}/kiosk/link?session={session_id}"
